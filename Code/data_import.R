...
for (i in 1:(length(sheets) - 1)) {
  ...
  if ((trainings |> summarise(sum = sum(sum)))$sum == 0) {
    next
  }
...
stamm <- stamm |>
  ...
  mutate(year = substr(year, 2, 5), VornameOriginal = Vorname) |>
  rename(
    n_years = JahreaktivimVerein,
    n_years_passiv = JahrepassivimVerein,
    since = Mitgliedseit
  ) |>
  mutate(across(c("year", "n_years", "n_years_passiv"), as.numeric)) |>
  filter(!is.na(ID)) |>
...
while (nrow(dups) > 0) {
  for (i in 1:nrow(dups)) {
    stamm[stamm$ID == dups$ID[1], ] <- stamm[stamm$ID == dups$ID[1], ] |>
      mutate(
        Vorname = paste0(VornameOriginal, " ", substr(Nachname, 1, 2), ".")
      )
  }
  dups <- find_dupes(stamm)
}
