...
cumvisits <- stats |>
  ...
  left_join(
    stats |>
      filter(status == "a") |>
      count(ID, wt = presence, name = "n_active"),
    by = "ID"
  ) |>
  arrange(-n)
...
actives <- stamm |>
  ...
  left_join(
    stamm |>
      ...
      group_by(ID) |>
      summarise(activeYearsSince2004 = n()),
    by = "ID"
  ) |>
...
# all members ever
all_members <- stamm |>
  filter(status == "a" | status == "p") |>
  distinct(ID, .keep_all = TRUE) |>
  select(ID, sex, status, n_years, n_years_passiv, Vorname, since) |>
  mutate(n_years_total = n_years + n_years_passiv) |>
  # for correct re-order in chart, factor has to be recreated
  arrange(desc(n_years_total)) |>
  mutate(Vorname = factor(Vorname, levels = Vorname))
...
figs <- tibble(
  cat = "Trainings",
  value = all_trainings$n,
  head = "seit 2004"
) |>
  add_row(cat = "Trainingsbesuche", value = visits$n, head = "seit 2004") |>
  add_row(
    cat = "max. Teilnehmerzahl",
    value = max(maxmin$max),
    head = "seit 2004"
  ) |>
  add_row(
    cat = "min. Teilnehmerzahl",
    value = min(maxmin$min),
    head = "seit 2004"
  ) |>
  add_row(
    cat = "Jahre Aktivmitgliedschaften",
    value = mitgliedschaftsjahre$n,
    head = "seit Gründung"
  ) |>
  add_row(
    cat = "bisherige Aktivmitglieder",
    value = nmembers$n,
    head = "seit Gründung"
  ) |>
  ...
  add_row(
    cat = "Frauenanteil",
    value = label_percent(accuracy = .1)(females$ratio),
    head = "seit Gründung",
    .after = 5
  ) |>
